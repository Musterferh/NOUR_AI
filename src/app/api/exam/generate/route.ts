...
export async function POST(req: Request) {
  ...
  try {
    ...
    release = await acquireLease(`${ownerId}:exam-generation`, 125_000);
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      let output: unknown;
      try {
        output = await callKimiJson(messages, { signal, maxTokens: 12000 });
      } catch (error) {
      ...
      try {
        ...
        if (attempt === 0) {
          messages.push({ role: 'user', content: 'The previous output did not meet the required schema or source checks. Generate a fresh JSON array with EXACTLY 20 distinct questions. Include all required fields, exactly four distinct options, a valid A/B/C/D answer, and only exact source IDs supplied above. Do not include id fields or additional properties.' });
        }
      }
    }
    if (!questions) throw new HttpError(502, 'A valid, fully sourced 20-question exam could not be generated. Please try again.');
