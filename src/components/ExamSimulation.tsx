  ...
  return <section className="workspace" aria-label="Exam simulator">
    ...
    <div className="workspace-scroll"><div className="content-column">
      ...
      {exam.loading ? <p className="empty-note" role="status">Checking for a saved exam…</p> : !attempt ? <div className="exam-intro card">
        <div className="brand-orb"><Clock size={32} /></div><p className="eyebrow">MAKE PRACTICE COUNT</p><h2>Ready for your next challenge?</h2><p>A 20-question exam on <strong>{category}</strong>. You have 30 minutes. Your answers save as you go, and the timer continues if you leave or close the page.</p>
        {Boolean(focusTopics?.length) && <p className="focus-topics">Revision focus: {focusTopics?.join(', ')}</p>}
        <div className="exam-facts"><span><strong>20</strong> questions</span><span><strong>30</strong> minutes</span><span><strong>90%</strong> study target</span></div>
